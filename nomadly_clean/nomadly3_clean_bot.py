...
class NomadlyCleanBot:
    ...
    async def handle_callback_query(self, update: Update, context):
        ...
        try:
            ...
            elif data and data.startswith("crypto_trx_"):
                domain = data.replace("crypto_trx_", "")
                await self.handle_crypto_address(query, "trx", domain)

            elif data and data.startswith("crypto_bsc_"):
                domain = data.replace("crypto_bsc_", "")
                await self.handle_crypto_address(query, "bsc", domain)

            elif data and data.startswith("crypto_bch_"):
                domain = data.replace("crypto_bch_", "")
                await self.handle_crypto_address(query, "bch", domain)

            elif data and data.startswith("crypto_usdt_"):
                domain = data.replace("crypto_usdt_", "")
                await self.handle_crypto_address(query, "usdt", domain)

            elif data and data.startswith("crypto_ustcr_"):
                domain = data.replace("crypto_ustcr_", "")
                await self.handle_crypto_address(query, "ustcr", domain)
    ...
    async def show_wallet_funding_options(self, query):
        ...
        try:
            ...
            keyboard = [
                ...
                [
                    InlineKeyboardButton(texts["trx"], callback_data=f"fund_crypto_trx"),
                    InlineKeyboardButton(texts["bsc"], callback_data=f"fund_crypto_bsc")
                ],
                [
                    InlineKeyboardButton(texts["bch"], callback_data=f"fund_crypto_bch"),
                    InlineKeyboardButton(texts["ustcr"], callback_data=f"fund_crypto_ustcr")
                ],
                [
                    InlineKeyboardButton(texts["usdt"], callback_data=f"fund_crypto_usdt")
                ],
                [
        ...
        try:
            ...
            if wallet_balance >= price:
                ...
                keyboard = [
                    ...
                    [
                        InlineKeyboardButton(texts["trx"], callback_data=f"crypto_trx_{domain}"),
                        InlineKeyboardButton(texts["bsc"], callback_data=f"crypto_bsc_{domain}")
                    ],
                    [
                        InlineKeyboardButton(texts["bch"], callback_data=f"crypto_bch_{domain}"),
                        InlineKeyboardButton(texts["ustcr"], callback_data=f"crypto_ustcr_{domain}")
                    ],
                    [
                        InlineKeyboardButton(texts["usdt"], callback_data=f"crypto_usdt_{domain}"),
                        InlineKeyboardButton(texts["fund_wallet"], callback_data="fund_wallet")
                    ],
                    [
    ...
    async def handle_payment_selection(self, query, domain):
        ...
        try:
            ...
            keyboard.extend([
                ...
                [
                    InlineKeyboardButton("⟠ Tron", callback_data=f"crypto_trx_{domain}"),
                    InlineKeyboardButton("♦ Binance BSC", callback_data=f"crypto_bsc_{domain}")
                ],
                [
                    InlineKeyboardButton("Ƀ Bitcoin Cash", callback_data=f"crypto_bch_{domain}"),
                    InlineKeyboardButton("₮ Tether ERC20", callback_data=f"crypto_ustcr_{domain}")
                ],
                [
                    InlineKeyboardButton("₮ Tether TRC20", callback_data=f"crypto_usdt_{domain}")
                ],
                [
    ...
    async def handle_crypto_address(self, query, crypto_type, domain):
        ...
        try:
            ...
            crypto_info = {
                ...
                'trx': {'name': 'Tron', 'symbol': '⟠', 'confirmations': '20 blocks (~20 min)'},
                'bsc': {'name': 'Binance BSC', 'symbol': '♦', 'confirmations': '20 blocks (~20 min)'},
                'bch': {'name': 'Bitcoin Cash', 'symbol': 'Ƀ', 'confirmations': '20 blocks (~20 min)'},
            ...
            try:
                ...
                if user_id in self.user_sessions:
                    ...
                    self.user_sessions[user_id]['payment_qr'] = address_response.get('data', {}).get('qr_code')
                ...
                error_texts = {
                    ...
                    "fr": "❌ Erreur du Système de Paiement\n\nImpossible de générer l'adresse de paiement. Veuillez contacter le support.\n\nErreur: API BlockBee indisponible",
                    "hi": "❌ भुगतान सिस्टम त्रुटि\n\nभुगतान पता उत्पन्न करने में असमर्थ। कृपया समर्थन से संपर्क करें।\n\nत्रुटि: BlockBee API अनुपलब्ध",
                    "zh": "❌ 支付系统错误\n\n无法生成付款地址。请联系支持。\n\n错误：BlockBee API 不可用",
                ...
                await query.edit_message_text(
                    ...
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton(back_texts.get(user_lang, back_texts["en"]), callback_data="payment")]
                    ])
                )
                return
