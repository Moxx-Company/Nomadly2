            ...
            try:
                ...
                try:
                    ...
                    if db_url:
                        ...
                        # SECURE PAYMENT VERIFICATION: Only process real blockchain confirmations
                        logger.debug(f"Checking payment status for {address[:20]}...")
                        
                        # Skip database-based verification - BlockBee uses webhooks for real confirmations
                        # This prevents false positives from database status checks
                            
                        cursor.close()
                        conn.close()
                        
                    # REMOVED DANGEROUS PAYMENT CONFIRMATIONS FILE CHECK
                    # This was creating false positives - BlockBee uses webhooks for real confirmations
                except Exception as e:
